 ...
 *	@param bUpdateDB：可选参数，是否更新数据库
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_SetPayload(unsigned int boxID, unsigned int rbtID, double dMass, double dX, double dY, double dZ, unsigned char byOption=0);
/**
 ...
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param dPoseFrom_X~Rz : Pose of oringinal point / 源点空间座标
 *	@param dPoseTo_X~Rz : Pose of calculation result / 逆运算后的空间座标
 *
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */	                       
...
/***
 *	@brief: Calculating the arc passing through two intermediate points using three-point teaching / 三点示教圆弧计算两个中间经过点的接口
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
...
/**
 *	@brief: Set the end freedom in free drive / 设置拖动示教时的末端自由度
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
...
/**
 *	@brief: Start waypoint move. HRIF_MoveJ or HRIF_MoveL is more recommended yet / 执行路点运动-建议直接使用HRIF_MoveJ和HRIF_MoveL
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
...
/**
 *	@brief: Start joint move / 机器人运动到指定的角度坐标位置
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param dX~Rz : Target pose, invalid when IsUseJoint equals 1. Target joint positions is obtained through inversely solving from this pose when IsUseJoint equals 0 / 目标迪卡尔位置-当nIsUseJoint=1时无效，nIsUseJoint=0时，用此迪卡尔坐标作为目标位置，通过逆解计算得到关节坐标为目标关节坐标
 *	@param dJ1~6 : Target joint positions when IsUseJoint equals 1; Reference joint positions for the inversely solving when IsUseJoint equals 0. / 目标关节位置-当nIsUseJoint=1时，使用此关节坐标作为目标关节坐标，nIsUseJoint=0时，此关节坐标仅作为计算逆解时选解的参考关节坐标
...
/**
 *	@brief: Start linear move / 机器人直线运动到指定的空间坐标位置
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 ...
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param nPoseId: Pose ID, 0~5 for X, Y, Z, Rx, Ry, Rz / 空间座标维度编号
 *	@param nDirection: Move direction, 0 for negative, 1 for positive / 运动方向，1为正方向，0为负方向
 *	@param dDistance: Move distance (mm or °) / 运动距离(mm或者°)
 *  @param nToolMotion : Tool motion, 0 for UCS based move, 1 for TCP based move / 运动座标类型，0为按当前UCS，1为当前TCP
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
...
/**
 *	@brief: Start to push points of joint positions for joint path move / 初始化关节连续轨迹运动
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param sTrackName : Track name / 轨迹名称
...
/**
 *	@brief: Push an point of joint positions to track end of sTrackName for joint path move. More than 4 points are needed. / 添加运动轨迹点，可多次调用此函数，一般情况下点位数量需要>4
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param sTrackName : Track name / 轨迹名称
...
/**
 *	@brief: Finish pushing points of joint positions for joint path move and begin to calculate the track / 轨迹下发完成，开始计算轨迹
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param sTrackName : Track name / 轨迹名称
 *	
 ...
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_EndPushMovePath(unsigned int boxID, unsigned int rbtID, string sTrackName);

/**
 *	@brief: Start joint path move / 执行指定的轨迹运动
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param sTrackName : Track name / 轨迹名称
 *	
...
/**
 *	@brief: Read track status of joint path move / 读取当前的轨迹状态
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param sTrackName : Track name / 轨迹名称
 *	@param nResult : Current track status / 当前轨迹状态
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
...
/**
 *	@brief: Update track name for joint path move / 更新指定轨迹的名称
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param sTrackName : Original track name / 轨迹原名称
 *	@param sTrackNewName : New track name / 需要更新的轨迹名称
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
...
/**
 *	@brief: Delete a track of joint path move / 删除指定轨迹
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param sTrackName : Track name / 需要删除的轨迹名称
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
...
/**
 *	@brief: Set Override for joint path move / 设置 MovePath 运动速度比
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
...
/**
 *	@brief: Read process and point index of a path move / 读取轨迹进度
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param dProcess : Current track process, 0~1, more than 0.999999 for move done / 当前运动进度(0-1),>0.999999表示运动完成
...
/**
 *	@brief: Initialize linear path move. For the whole procedure of linear path move, you should call this function firstly to initialze a track, then call HRIF_PushMovePaths or HRIF_PushMovePathL to push pose points to the track, finally call HRIF_MovePathL to start path calculation which needs several seconds depending on points number and subsequently move if calculation is successful. / 初始化迪卡尔坐标轨迹运动。迪卡尔坐标轨迹运行需要先调用HRIF_InitMovePathL，再调用HRIF_PushMovePaths或HRIF_PushMovePathL添加点位，然后调用HRIF_MovePathL，系统会自动先计算轨迹，成功后直接开始运动，计算时间2-4s左右，根据实际轨迹大小而定。
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param sTrackName : Track name / 轨迹名称。
...
/**
 *	@brief: Push an pose point to track end of sTrackName for linear path move / 下发轨迹点位-调用一次下发一个目标点位
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param trackName : Track name / 轨迹名称
 *	@param dX~Rz : Target pose / 目标迪卡尔坐标位置 
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
...
/**
 *	@brief: Push an list of pose points to track end of sTrackName for linear or joint path move /  批量下发轨迹点位-调用一次可下发多个点位数据
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param trackName : Track name / 轨迹名称
 *	@param nMoveType : Move type, 0 for MovePathJ, 1 for MovePathL / 运动类型-MovePathJ可以共用 0(MovePathJ)/1(MovePathL)
 *	@param nPointsSize : Points number / 轨迹点位数量
 *	@param sPoints : Points list, separated by ","  / 轨迹点列表，以","分隔
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
...
/**
 *	@brief: Start linear path move 执行迪卡尔坐标轨迹运动，
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param trackName : Track name / 轨迹名称
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 ...
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
int HRIF_GetComplianceFactor(unsigned int boxID, unsigned int rbtID, double& Linear, double& Angular);
 ...
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
int HRIF_SetFTFreeFactor(unsigned int boxID, unsigned int rbtID, double Linear, double Angular);
